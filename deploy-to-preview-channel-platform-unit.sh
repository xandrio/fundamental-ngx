...
DEPLOY_TO_PREVIEW_CHANNEL_RESULT=$(firebase hosting:channel:deploy unit-platform-pr-$TRAVIS_PULL_REQUEST --expires 1.5d --token $FIREBASE_TOKEN --json)

RESULT=`echo ${DEPLOY_TO_PREVIEW_CHANNEL_RESULT} | jq -r '.result'`
ALLURE_RESULTS=`echo ${RESULT} | jq -r '."unit-tests-allure-report"'`
